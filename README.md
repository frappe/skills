...
| `technical-writing` | Write documentation, READMEs, commits, pull requests, and release notes in Simplified Technical English. User-invoked only: run `/technical-writing` |
...
Most skills activate automatically when you ask your agent about a matching task. Examples: create a DocType, build a Vue SPA, or run `bench migrate` (`frappe-app-dev`), review a diff (`quality-code-review`), or lay out a page (`ui-design`).

Some skills only run when you call them. Run `/technical-writing`, `/draft-security-advisory`, or `/resolve-backport-conflicts` to start those skills.
